...
def recommend(
    user_id: str, k: int = 10, recent_clicks=None, locale="en"
) -> List[Dict[str, Any]]:
    ensure_data()
        ...
        result.append(
            {
                ...
                "title": article.get("Title", ""),
                "reason": f"Matches search query: {q}"
...
def summarize(
    title: Optional[str], abstract: Optional[str], max_tokens: int = 128
) -> str:
    ...
    try:
        ...
        out = summarizer(
            ...
            min_length=max(32, max_tokens // 4),
            do_sample=False,
        )
